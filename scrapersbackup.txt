...
builder.defineStreamHandler(async (args) => {
    ...
    try {
        // 1. Get Episode ID from Details (Uses Cache if available)
        ...
        // 2. Fetch Video Link (NO CACHE - Must be fresh, Longer Timeout)
        const videoData = await fetchWithFlare(videoApiUrl, false, 40000);
        
        if (videoData && videoData.Video) {
            console.log(`[v26] ✅ Video Link Found!`);
            return {
                streams: [{
                    name: "⚡ SojuStream",
            ...
            console.error(`[v26] ❌ Stream Error: API returned no video link. Result: ${JSON.stringify(videoData)}`);
        }
    } catch (e) {
